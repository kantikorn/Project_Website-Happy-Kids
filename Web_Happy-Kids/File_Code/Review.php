...
<?php 
  $sql_reviews = $conn->query("SELECT tbl_order.*,tbl_cart.*,tbl_product.* FROM tbl_order LEFT JOIN tbl_cart ON tbl_order.order_id = tbl_cart.order_id LEFT JOIN tbl_product ON tbl_cart.ca_product = tbl_product.product_id WHERE tbl_order.cus_name = '".$_SESSION['user_id']."' AND tbl_order.order_status = '2' AND tbl_order.order_status = '4' ");
?>
