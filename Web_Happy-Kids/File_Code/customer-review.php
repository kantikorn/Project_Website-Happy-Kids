...
<div class="container-fluid mt-3">
    ...
    <section class="mt-4">
      <?php 
        $sql_cus_reviews = $conn->query("SELECT tbl_review.*,tbl_user.*,tbl_cart.ca_product FROM tbl_review  INNER JOIN tbl_user ON tbl_review.re_user = tbl_user.id INNER JOIN tbl_cart ON tbl_review.order_id = tbl_cart.order_id");
        
      ?>
        <div class="row text-center d-flex align-items-stretch">
          <?php 
            while ($result = $sql_cus_reviews->fetch_object()) {
              $sql = $conn->query("SELECT * FROM tbl_product WHERE product_id='".$result->ca_product."' ");
              while ($fet_pro = $sql->fetch_object()) {
               
          ?>
          <div class="col-md-4 mb-5 mb-md-0 d-flex align-items-stretch">
            <div class="card testimonial-card w-75 d-flex mx-5 mt-5">
              <div class="card-up" style="background-color: #9d789b;"></div>
              <div class="avatar mx-auto bg-white">
                <img src="image/product/<?php echo $fet_pro->imgproduct;?>"
              ...
              <div class="card-body">
                ...
                <p class="card-text">รีวิวเมื่อวันที่: <?php echo $result->re_date;?></p>
              </div>
            </div>
          </div>
          <?php }?>
