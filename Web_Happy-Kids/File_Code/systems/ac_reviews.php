...
session_start();
    ...
    if (isset($_REQUEST['ac'])) {
        switch ($_REQUEST['ac']) {
            case 'addnew':
                $sql = $conn->query("INSERT INTO tbl_review(re_user,feedback,comment,re_date,order_id) VALUES('".$_SESSION['user_id']."','".$_REQUEST['feedback']."','".$_REQUEST['comment']."',now(),'".$_REQUEST['order_id']."') ");
                if ($sql) {
                    echo '<script>
                    setTimeout(function(){
                        swal({
